...
두 템플릿과 `propose_neon_01` 은 `love-you-586695.mp3`,
`wedding_film_01` 은 `wedding-music-574002.mp3` 를 쓴다.
`birthday_pop_01` · `anniversary_rose_01` 은 톤이 맞지 않아 그대로 두었다 —
플레이스홀더 합성음이라 `check --sale` 이 계속 막는다.
...
인화지는 기울어진 채 고정되고 그 안의 사진만 움직인다. `motion.zoom` 은
Ken Burns 의 시작/끝 배율이고, `anchor` 가 확대 기준점이다.

강도를 액자 크기와 **동시에 움직이는 장수** 로 나눠 잡는다.

| 씬 | zoom | 이유 |
|---|---|---|
| story / opening / proposal | 1.0 → 1.12~1.14 | 액자 1~2장, 또렷하게 보여야 한다 |
| gallery (큰 사진 한 장) | 1.0 → 1.11 | 액자가 커서 같은 배율도 크게 보인다 |
| montage (5장 동시) | 1.0 → 1.07 | 다섯 장이 같이 커지면 어지럽다 |

세로 액자(`frame_ar < 0.9`)는 `anchor: "top"` 을 쓴다. 인물 사진은 얼굴이
위쪽에 오는 경우가 많아 중앙 기준으로 당기면 얼굴이 잘린다.

⚠️ 줌을 키우면 **요구 해상도가 그만큼 올라간다.** preflight 의 목표 크기는
`박스 크기 × max(zoom)` 이라, 1.06 → 1.14 로 올리면 요구치가 7.5% 높아진다.
고객 사진이 휴대폰 원본이면 `check` 의 확대 배율 경고를 먼저 확인할 것.

⚠️ 전체화면 사진에서 쓰던 폭을 화면의 20~30%짜리 액자에 그대로 쓰면 과하게
보인다. 액자가 작을수록 같은 배율이 더 크게 느껴진다.
...
`assets/bgm/` 에 두고 템플릿에서 `bgm.src` 로 참조한다 (`tpl.asset()` 이
템플릿 폴더 → 공용 assets 순으로 찾는다).

기본 음원은 Pixabay Content License 다. **고객 납품 영상을 포함한 상업 영상에
쓸 수 있고 출처 표기도 필요 없다** — 음악이 더 큰 창작물의 일부로 들어가고
단독 파일로 재배포되지만 않으면 된다. 주문제작이 정확히 이 경우다.

⚠️ 다만 Pixabay 는 **면책 보상이 $0** 이고 기여자 업로드에 대한 저작권 검증이
없다. 문제가 생기면 판매자가 그대로 떠안는다. 다운로드 페이지 URL·날짜를
`assets/bgm/SOURCES.md` 에 남겨둘 것 — 분쟁 시 실질적인 방어 수단이다.
매출이 커지면 면책 보상이 붙는 Artlist·Epidemic Sound 로 옮기는 게 맞다.

⚠️ `bgm.gain_db` 는 사실상 무효다. 뒤따르는 `loudnorm=I=-14` 가 덮어쓰기
때문에 무엇을 넣든 출력은 -14 LUFS 로 맞춰진다. 음량을 바꾸려면 loudnorm 의
`I` 값을 건드려야 한다.
